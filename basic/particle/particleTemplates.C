...
)
{
    ...
    else
    {
        ...
        )
        {
            ...
            // Added isA<mappedWallPolyPatch>(patch) to be able to hit multi-region patch
            else if (isA<wallPolyPatch>(patch) || isA<mappedWallPolyPatch>(patch))
            {
                p.hitWallPatch
                (
    ...
    if (lambdaMin < SMALL)
    {
        ...
        )
        {
            ...
            // Added isA<mappedWallPolyPatch>(patches[patchI]) to be able to hit multi-region patch
            if (isA<wallPolyPatch>(patches[patchI]) || isA<mappedWallPolyPatch>(patches[patchI]))
            {
