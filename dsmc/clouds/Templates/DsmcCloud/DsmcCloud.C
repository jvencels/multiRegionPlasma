...
void Foam::DsmcCloud<ParcelType>::resetFields()
{
    ...
    rhoQ_ = dimensionedScalar("zero",  dimensionSet(0, -3, 1, 0, 0, 1, 0), VSMALL);
...
void Foam::DsmcCloud<ParcelType>::calculateFields()
{
    ...
    scalarField& rhoQ = rhoQ_.internalField();
...
(
    ...
    (
        ...
        mesh_,
        dimensionedScalar("zero",  dimensionSet(0, -3, 1, 0, 0, 1, 0), VSMALL)
    ),
    rhoM_
    (
        IOobject
